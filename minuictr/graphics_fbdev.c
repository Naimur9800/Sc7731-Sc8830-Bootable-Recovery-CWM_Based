...
static GRSurface* fbdev_flip(minui_backend* backend __unused) {
    ...
    if (gr_framebuffer[0].pixel_bytes == 4) {
        for (unsigned int y = 0; y < gr_draw->height; ++y) {
            uint32_t* dst = gr_framebuffer[gr_active_fb].data + y * row_pixels;
            uint32_t* src = gr_draw->data + (gr_draw->height - y - 1) * row_pixels + gr_draw->width;
            for (unsigned int x = 0; x < gr_draw->width; ++x)
                *(dst++) = *(--src);
        }
    } else {
        for (unsigned int y = 0; y < gr_draw->height; ++y) {
            uint16_t* dst = gr_framebuffer[gr_active_fb].data + y * row_pixels;
            uint16_t* src = gr_draw->data + (gr_draw->height - y - 1) * row_pixels + gr_draw->width;
            for (unsigned int x = 0; x < gr_draw->width; ++x)
                 *(dst++) = *(--src);
        }
    }
